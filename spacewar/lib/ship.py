...
class DoubleLazer(object):
    ...
    class Bullet(Rect,object):
        ...
        img = image.load(os.path.join(maindir, 'img/lazer1.png'))
...
class Ship(Rect,object):
    ...
    img          = image.load(os.path.join(maindir, 'img/xwing.png'))
    rect         = img.get_rect()
    #img2         = image.load('img/xwing2.png')
    img2         = image.load(os.path.join(maindir, 'img/xwing2.png'))
    msk          = mask.from_surface(img,1)
    #lazertempfx  = image.load('img/lazertemp.png')
    lazertempfx  = image.load(os.path.join(maindir, 'img/lazertemp.png'))
    shield = 1
    alive = True
